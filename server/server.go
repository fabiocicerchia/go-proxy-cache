...
func (s *Servers) InitServers(domain string, domainInitServer config.Server) {
	srv := InitServer(domain)
	s.AttachPlain(domainInitServer.Port.HTTP, srv)

	srvHTTPS := srvtls.ServerOverrides(domain, *srv, domainInitServer)
	s.AttachSecure(domainInitServer.Port.HTTPS, &srvHTTPS)
}
...
func (s *Servers) StartDomainServer(domain string) {
	...
	// redis connect
	circuit_breaker.InitCircuitBreaker(domain, domainConfig.CircuitBreaker)
