...
func StoreFullPage(url string, content string, expiration time.Duration) (bool, error) {
	...
	err := rdb.Set(ctx, "STATUS@@"+key, content, expiration).Err()
	if err != nil {
		return false, err
	}

	err = rdb.Set(ctx, "HEADERS@@"+key, content, expiration).Err()
	if err != nil {
		return false, err
	}
...
func RetrieveFullPage(url string) (status string, headers []string, content string, err error) {
	Connect()
	...
	headersList, err := rdb.Get(ctx, "HEADERS@@"+key).Result()
	if err != nil {
		return "", "", "", err
	}
